...
| D. 接口契约 | §接口契约-SPI / §接口契约-REST（REST 章节含 ①bis 6 维度） / §错误码 / §🔴前端错误码处理 | 每字段四维（来源/必要性/合理性/可行性）必填 |
...
| G. ①bis 前端接口契约 | §接口契约-REST（已内聚 6 维度，v3.10.9 拆分）/ §🔴状态流转前端展示 / §🔴边界场景前端处理 / §🔴前端错误码处理 | 6 维度全填，任一缺失 = 阻断 |
...
- [ ] 每个状态值 → 对应 DR §状态机
- [ ] 每个错误码 → 对应 DR §错误码表
- [ ] 每个复用项 -> 对应依赖 Story §接口契约-SPI / §接口契约-REST
