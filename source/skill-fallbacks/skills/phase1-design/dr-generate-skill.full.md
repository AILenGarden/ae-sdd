...
第零步：DR 准入检查（PRD + RA + 项目资产 + 产品原型 + dr-template + 约束）
    ↓
第一步：读取输入（继承 RA SRS 结论 + 重新组织为 DR 章节）
    > RA 通用性改造后，RA 产物是唯一自适应 SRS（`ae-sdd-ra-srs/v2`）。下表的
    > "RA 维度"应从新 SRS 对应位置读取，不要求旧固定章节编号：
    > 角色/权限→§8.1、场景→§8.2、状态/生命周期→§8.3、数据语义→§8.4、
    > 外部契约→§8.5、质量/安全/合规→§8.6、兼容/迁移/运行→§8.7；
    > 规范性需求从 §4 REQ-* 读、验收从 §5 AC-* 读、约束/风险/未决从 §6 读。
    ├─ 角色/权限（RA §8.1，仅 applicable）→ DR §5 架构概览 + §8 接口契约
    ├─ 业务规则（RA §4 REQ / §6 约束）→ DR §3 约束承接 + §9 业务规则
    ├─ 数据要素（RA §8.4，仅 applicable）→ DR §7 数据模型
    ├─ 业务流程（RA §8.2 / §8.3）→ DR §6 时序 + §9 状态机
    ├─ 设计方向 → DR §4 关键决策（DR 自身裁定，RA 不预选方案）
    └─ 场景分析（RA §8.2）→ DR §8 接口契约 + §10 故障模式
    ↓
...
|---|------|------|---------|
| 1 | 用户提供 | PRD 文档 | 全文 |
| 2 | 来自 requirement-analysis | RA 文档 | 全文（SRS Core、适用的条件章节与规模证据）|
...
## 第一步：读取输入（继承 RA SRS v2 结论）

> **🔴 核心思想：** DR 不是"重写" RA，DR 读取 `ae-sdd-ra-srs/v2` 的 REQ/AC、约束/风险/GAP、规模证据与状态为 applicable 的条件章节，再按 DR 模板组织并补充技术决策。RA 不预选实现方案，DR 不要求旧固定章节编号。

### 1.1 RA → DR 章节映射

| SRS v2 来源 | → DR 章节 | 映射动作 |
|---------|----------|---------|
| §4 REQ-* + §6 约束 | DR §3 约束承接 + §9 业务规则 | 保持需求身份和 source refs，补技术落点 |
| §5 AC-* | DR §13 测试策略 + 各 Story AC | 建立 Story × AC 覆盖，不改写验收语义 |
| §8.1 参与方/权限（applicable） | DR §5 架构概览 + §8/§11 | 映射职责、接口与授权边界 |
| §8.2 场景（applicable） | DR §6/§8/§10 | 映射主、异常与边界交互 |
| §8.3 状态/生命周期（applicable） | DR §6/§9 | 设计跨 Story 状态和时序 |
| §8.4 数据语义（applicable） | DR §7 | 设计实体、字段与不变量的技术承接 |
| §8.5 外部契约（applicable） | DR §8/§10 | 冻结接口与失败语义 |
| §8.6 质量/安全/合规（applicable） | DR §3/§11/§13/§14 | 形成可验证质量设计 |
| §8.7 兼容/迁移/运行（applicable） | DR §15/§16 | 设计发布、迁移、回滚与风险缓解 |
| §6 假设/风险/GAP | DR §16/§17 | 已关闭项保留追踪；未决项不得被技术假设掩盖 |
| §7 规模裁定 | DR §12 | 约束 Story 深度；不以旧章节或实现数量重算规模 |
...
- 目标 1：{从 SRS §1 问题与目标、§4 REQ 提炼}
- 目标 2：{从 SRS §7 规模证据与 §5 AC 提炼}
- 非目标：{SRS §1 非目标显式声明}
- 范围（包含/不包含）：{从 SRS §2 继承}
...
|---------|---------|--------------------|------|
| technology-stack.md | | | {cite SRS §6 已冻结约束；无则直接引用 constraints owner} |
...
**背景：** {cite SRS §4 REQ / §6 约束风险；实现方案由本 DR 裁定}
...
| 闸 | 名称 | 等级 | 判定 SOP |
|----|------|------|---------|
| 1 | SRS v2 全承接闸 | 🔴 阻断 | DR 覆盖全部 REQ/AC、§6 约束风险/GAP 与每个 applicable 条件章节；技术决策由 DR 产生 |
...
| 20 | §闸 1：SRS v2 全承接闸 | 闸门 1 通过 | REQ/AC/适用条件章节 0 遗漏 |
...
| 5 | 禁止 DR §12 Story 矩阵与 SRS §7 规模裁定不一致 | 规模不一致 = 流程错乱 | §第三步 自检 |
