...
- [ ] **唯一 SRS**（`intent=RA` 单文档；不生成 PRD/Issue 前置，不生成旁车）
- [ ] **SRS Core 完整**（§0 身份+来源、§1 问题/目标/非目标、§2 范围、§3 适用性、§4 需求、§5 验收、§6 约束/风险/gap、§7 规模）
- [ ] **适用性闭合**（七维均已判定；applicable 有章节；unknown 无遗留阻断 GAP）
- [ ] **可追溯与可验收**（每条 REQ 有 REF 来源、有 AC 覆盖）
- [ ] **缺口管理**（blocking GAP 全部关闭，否则 analysisState 保持 draft）
- [ ] **纯需求规模裁定**（六维评分取最高分；不引用文件/类/表数量；header Scale 与 §7 一致）
- [ ] **一次最终批准**（用户批准由 daemon receipt 持有，不回写 SRS 正文）
- [ ] **落盘**（`ae-sdd doc save --intent RA`，触发 G-RA-1~G-RA-4）

### 步骤 4：用户催促的反模式处理

若用户催促"我想要一次性回答 / 不想多次回答"，**禁止**直接接受。必须：

1. 提示"RA skill 的最小合规路径"（步骤 3 的硬卡片）
2. 把所有 🔴 阻断型问题一次性列给用户
