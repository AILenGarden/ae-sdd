...
version: 3.10.1
description: |
  端到端自动化工程主入口（v3.10.1）。从 DR 出发，经 Story->TestCase->CodingPlan->Coding->Test->Review，直到全部通过。
...
source_fallback_sha256: 585a5c7eb58940baa5fadd255643e4200fc6dd57f36cbb4d35c3cd6760321c78
source_original_bytes: 34057
source_original_lines: 701
source_semantic_inventory_sha256: 6a2b9d6d67fa312a5235353512d45599bb9e44fc41421f3e8f81403b0a504de1
...
- fallback_sha256: `76ada6cdf39c449cafd4697eeff58a6bbdeabdc79a7170f557372e4a0dd9459f`
- original_lines: 701
- original_bytes: 34057
- semantic_inventory_sha256: `6a2b9d6d67fa312a5235353512d45599bb9e44fc41421f3e8f81403b0a504de1`
- standard: `standards/skill-source-slimming-standard.md`
- template: `templates/skill/source-skill-slim-entry-template.md`
- summary: 端到端自动化工程主入口（v3.10.0）。从 DR 出发，经 Story->TestCase->CodingPlan->Coding->Test->Review，直到全部通过。
支持大/中/小/微四条子链（按已有产物就近入链）、流程状态跟踪、中断恢复、主流程监管器（产物核查+偏移检测+暂离回归协议）。
...
| --- | --- | --- | --- |
| identity_trigger | frontmatter: name, description, version; headings: L3:112 编码意图检测（暂离期间触发）; L3:143 G-00 项目资产（每次调用必过）; keyword_hits: 27 | source/docs/ae-sdd-design.md §2/§16/§18; source/docs/skill-runtime-compiler.md §2 | Keep frontmatter and summary in the slim entry; full trigger wording stays in fallback. |
| workflow_route | headings: L2:68 🎛️ 主流程监管器执行协议; L2:93 🔀 暂离与回归协议（流程偏离防护）; L3:262 开工前信息预收集（Step 1.5，仅自动化模式）; +9 more; keyword_hits: 116 | source/docs/ae-sdd-design.md §2/§16; source/standards/update-graph.json | Index the route/workflow outline; load fallback before executing low-frequency branch detail. |
| gate_constraint | headings: L2:141 🛡️ 门禁速查; L3:143 G-00 项目资产（每次调用必过）; L3:154 G-RA 需求分析准入（dr-generate / story-generate / task-generate 前必过）; +11 more; keyword_hits: 139 | source/docs/ae-sdd-design.md §5; tools/lib/gates.py:GATE_REGISTRY | Preserve gate identifiers in index; CLI gate output remains higher authority than prose. |
| tool_command | headings: L3:460 ①bis 前端视角接口审视; L2:606 🛠️ 工具 API 速查; keyword_hits: 78 | source/docs/ae-sdd-implementation-architecture.md §4/§5; source/docs/ae-sdd-design.md §13 | Index command/API references; full invocation contracts stay in fallback or implementation docs. |
| state_data | headings: L3:236 配置（`.ae-sdd/config.yaml` 的 `automation` 段，SSOT）; L3:338 状态机子链（实际 state.json phase 值）; L2:437 流程状态与再启动; +3 more; keyword_hits: 68 | source/docs/ae-sdd-design.md §3/§15/§19; tools/lib/state.py | Index state/config vocabulary; use tools/lib state output as execution truth. |
| output_doc_contract | headings: L3:97 暂离声明（AI 主动输出）; L3:184 G-DOC-STORAGE（任何产出文档落地前）; L3:196 G-DOC-CONSISTENCY（文档落地前 + G-00后）; +4 more; keyword_hits: 67 | source/docs/ae-sdd-design.md §7; source/templates/** | Index document/output obligations; load fallback before generating exact long-form artifacts. |
| resource_reference | inline_refs: 77; refs: .ae-sdd/; .ae-sdd/compact-trigger; .ae-sdd/config.yaml; +74 more; keyword_hits: 67 | source/standards/**; source/templates/**; source/skills/** | Preserve referenced paths in the slim entry; copied fallback remains the semantic anchor. |
| design_alignment | headings: L2:294 🔴 实现方案决策基线（Story→Task→Coding 全链路）; L3:473 🔍 审核点1.5 实现方案预确认（AI先答，用户确认）; keyword_hits: 52 | source/docs/ae-sdd-design.md; source/docs/ae-sdd-implementation-architecture.md; source/docs/skill-runtime-compiler.md | Index the alignment surface; update design docs before changing behavior. |
...
| 3 | 124 | 回归门（回归时强制执行） |
| 2 | 141 | 🛡️ 门禁速查 |
| 3 | 143 | G-00 项目资产（每次调用必过） |
| 3 | 154 | G-RA 需求分析准入（dr-generate / story-generate / task-generate 前必过） |
| 3 | 171 | G-CODEPLAN-SRC（CodingPlan → ⑤Coding 前） |
| 3 | 184 | G-DOC-STORAGE（任何产出文档落地前） |
| 3 | 196 | G-DOC-CONSISTENCY（文档落地前 + G-00后） |
| 3 | 204 | G-14 CodingPlan-Story 一致性（CodingPlan → ⑤Coding 前，与 G-CODEPLAN-SRC 正交） |
| 3 | 216 | G-AUTO-CONSENSUS 自动化联审共识（🆕 v3.8.0，仅自动化模式启用） |
| 2 | 232 | 🚀 自动化模式（🆕 v3.8.0 — 输入→结果全自动化） |
| 3 | 236 | 配置（`.ae-sdd/config.yaml` 的 `automation` 段，SSOT） |
| 3 | 248 | 行为分叉（每个审核点） |
| 3 | 256 | 阻断出口 |
| 3 | 262 | 开工前信息预收集（Step 1.5，仅自动化模式） |
| 3 | 274 | 禁止事项（自动化模式专属） |
| 2 | 284 | 🔴 输出核心原则（最高优先级） |
| 2 | 294 | 🔴 实现方案决策基线（Story→Task→Coding 全链路） |
| 2 | 305 | 🎯 智能路由 |
| 3 | 309 | 路由表（编码类） |
| 3 | 329 | 4类规格判定 |
| 3 | 338 | 状态机子链（实际 state.json phase 值） |
| 2 | 349 | 📖 人工审核主动讲解规范 |
| 2 | 363 | 🤖 多 Agent 机制摘要 |
| 2 | 383 | ⏱️ 节点级上下文压力（6个审核点边界必调） |
| 2 | 400 | 整体流程骨架 |
| 2 | 437 | 流程状态与再启动 |
| 2 | 458 | Phase 1 关键节点 |
| 3 | 460 | ①bis 前端视角接口审视 |
| 3 | 465 | ② Story Review |
| 3 | 470 | 🔍 审核点1 对话内呈现（必须直接输出） |
| 3 | 473 | 🔍 审核点1.5 实现方案预确认（AI先答，用户确认） |
| 2 | 478 | Phase 2 关键节点 |
| 3 | 480 | ④bis CodingProcess（🆕 v3.5.17） |
| 3 | 498 | 🔍 审核点2 逐文件核对（强制，禁止一锅端） |
| 3 | 501 | 🔍 审核点2.5 CodingPlan评审（必须直接输出） |
| 2 | 506 | Phase 3 关键节点 |
| 3 | 508 | ⑥ 完成判定（6.1~6.10） |
| 3 | 523 | ⑦ter 流程收尾合规自检（5维度，禁止裸✅收尾） |
| 3 | 535 | ⑧ 完成产出物 |
| 2 | 551 | PRD级完成判定（v3.3.0） |
| 2 | 576 | 子 SKILL 索引 |
| 2 | 606 | 🛠️ 工具 API 速查 |
| 2 | 639 | 🔧 维护工作流 |
| 2 | 653 | 禁止事项 |
| 2 | 668 | 执行清单（TodoWrite 1:1 映射） |
