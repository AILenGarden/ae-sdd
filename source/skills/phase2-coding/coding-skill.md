...
source_fallback_sha256: 70c10d05718925af99d3b0fe3baf810df30c10a15ca03eb85defcdc0061bf331
...
- fallback_sha256: `70c10d05718925af99d3b0fe3baf810df30c10a15ca03eb85defcdc0061bf331`
- original_lines: 685
- original_bytes: 43634
- semantic_inventory_sha256: `61ae02ebd16dfa9ddcb5788a70532425ad76692ad61b86157ecb46612767cf8c`
...
| fallback_only_detail | keyword_hits: 12 | source/skill-fallbacks/**; source/CHANGELOG/** | Do not summarize aggressively; keep only the location signal and rely on fallback for exact detail. |
...
| 4 | 168 | 章节 1：文件级实现顺序 |
| 4 | 183 | 章节 2：关键类骨架 |
| 4 | 208 | 章节 3：数据结构 / DO 字段 |
| 4 | 216 | 章节 4：Mapper / Repository 关键 SQL |
| 4 | 225 | 章节 5：测试用例的对应实现 |
| 4 | 236 | 章节 6：编译与测试验证点 |
| 4 | 248 | 章节 7：调试与回滚策略 |
| 3 | 258 | §5.2 CodePlan 门禁（未通过禁止进入 Execute） |
| 2 | 280 | §6 CodeAnalysis ④bis 实战 SOP：分层拆分 + 项目资产映射方法论 |
| 3 | 289 | 核心设计哲学 |
| 3 | 295 | 方法论要素（CodeAnalysis 必须覆盖） |
| 4 | 297 | 要素 1：读取项目资产 |
| 4 | 303 | 要素 2：Task 执行顺序编排（不重写实现） |
| 4 | 309 | 要素 3：按抽象 4 层对每个 Task 做分层归类（🔴 核心） |
| 4 | 327 | 要素 4：把每个类按项目分层映射到确切包路径 |
| 4 | 333 | 要素 5：输出类骨架 |
| 3 | 337 | 7 条禁令（🔴 任何一条违反 = 整 Plan 打回） |
| 2 | 351 | §7 G-CODEPLAN-SRC 源码核对判定标准（🆕 v3.4.0） |
| 2 | 382 | §8 验证判定标准（Execute 阶段怎么算验证通过） |
| 3 | 386 | §8.1 编译验证 |
| 3 | 395 | §8.2 服务启动验证 |
| 3 | 416 | §8.3 主流程接口测试 |
| 3 | 425 | §8.4 错误码映射验证 |
| 3 | 434 | §8.5 DB 写操作落库验证 |
| 3 | 441 | §8.6 事务边界验证 |
| 2 | 447 | §9 编码后漂移核查 + 假修复识别 |
| 3 | 449 | §9.1 全切面一致性核查（漂移判定） |
| 3 | 474 | §9.2 假修复识别规则（测试造假判定） |
| 2 | 490 | §10 异常根因 4 层分类判定 |
| 2 | 510 | §11 经验检查清单 + 禁止事项红线 |
| 3 | 512 | §11.1 经验检查清单（通用，每次生成代码前逐项确认） |
| 3 | 534 | §11.2 禁止事项红线 |
| 3 | 555 | §11.3 基准过滤器自检（7 项，参考 be-coding-thinking-engine） |
| 2 | 567 | §12 静态扫描规则（通用 grep，Execute 编码后必跑） |
| 1 | 572 | 1. 标准库全限定名扫描（除 import 块外不应出现） |
| 1 | 576 | 期望输出为空；非空 → 修改为已 import 的短名 |
| 1 | 578 | 2. 未使用 import 扫描（IDE 警告即可） |
| 1 | 579 | IntelliJ: Code → Optimize Imports |
| 1 | 580 | Eclipse: Source → Organize Imports |
| 1 | 582 | 3. 静态导入滥用扫描 |
| 1 | 584 | 项目中应有节制使用，不应过多 |
| 2 | 594 | §13 语言/项目适配器注册加载（🆕 v3.6.1） |
| 3 | 600 | §13.1 加载协议 SOP（调用方执行，典型在 CodingProcess §A1 加载上下文时） |
| 3 | 628 | §13.1bis 叠加视图速查表（🆕 v3.6.2 — 降低 AI 脑内合并负担） |
| 3 | 651 | §13.2 适配器契约（注册进注册表的语言/项目 SKILL 必须满足） |
| 3 | 661 | §13.3 调用方与触发时机 |
| 3 | 668 | §13.4 零破坏声明 |
