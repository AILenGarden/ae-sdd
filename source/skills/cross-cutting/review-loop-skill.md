...
| 退出阈值 | 计数器累计到 **3**（即连续 3 轮无新增）→ 满足退出条件 |
| 缺陷定义 | "确认缺陷"= 经判定为 🔴 阻断型 / 🟠 严重型 / 🟡 一般型的缺陷（🟢 建议型不计入计数器归零，但需记录）|
| 节点专属硬门禁 | 各 review 节点可在退出条件上叠加**专属硬门禁**（如 story-review 的 C8 数据视角总览），未满足硬门禁不得退出 |

> **为什么是 3 轮？** 1 轮太容易审漏（AI 偶然一次没发现问题就退出）；2 轮是之前 story-review/dr-review 的设定但与 generate 侧 1 轮不一致；3 轮给 AI 最多自纠错机会，与循环上限 3 轮对齐（达到上限仍有 🔴 就升级用户，不再循环）。

### 协议 2：循环上限

**3 轮循环上限**。3 轮后仍有 🔴 阻断型缺陷未解决 → **升级用户决策**，不无限循环。

| 规则 | 说明 |
|---|---|
| 上限值 | **3** 轮（与退出阈值对齐）|
| 触发升级 | 第 3 轮结束仍有 🔴 阻断型缺陷 → 升级用户 |
| 升级动作 | 暂停循环，向用户输出"3 轮循环仍有 🔴：{缺陷清单}，请决策（人工介入 / 调整需求 / 放弃）" |
| 禁止 | 禁止无限循环（无上限的 review 是安全漏洞，v3.4.3 修复 task-generate TR 的此问题）|

> **退出条件 vs 循环上限的关系：**
> - 退出条件（连续 3 轮无新增）= **正常退出**，AI 自评估达标
> - 循环上限（3 轮仍有 🔴）= **异常退出**，AI 自评估不达标，交人决策
