...
从 PRD/Issue/对话需求生成 **完整、可评审、可路由、可落地实现** 的需求分析（RA）文档，7 大目标：
...
| **RA-06 数据语义、数据源与所有权** | 哪些实体/字段/外部数据源参与，谁创建、修改、读取，权威源是什么？ | 实体关系 + 字段约束 + 数据源清单 + 所有权 + 权威源 | 字段凭空编造，未用 `ae-sdd assets query`/DB/API/MQ/cache/config 证据反查 |
| **RA-07 业务规则与衍生规则** | 主规则触发后会衍生哪些规则？ | 主规则表 + R' 衍生规则登记表 | 状态变更类需求未跑 E.5 |
| **RA-08 跨域级联与数据流** | 状态/规则是否影响其他域、缓存、MQ、外部系统，数据如何从入口流到落点？ | 跨域级联效应表 + 数据流链路 | 涉及跨服务/缓存/MQ 却写"无影响"或无 source→sink 链路 |
| **RA-09 现有能力复用与实现证据** | 项目里是否已有模块、接口、组件、表、规则可复用？ | 复用扫描表 + 代码/表/API/assets/git 证据 + 不复用理由 | 未查 `ae-sdd assets query`（search/component/api）或无复用/改造/新建结论 |
| **RA-10 非功能、约束与成本可行性** | 性能、安全、合规、审计、可观测是否有要求？是否存在成本过高/难实现方案？ | 非功能约束矩阵 + 高成本设计反驳表 | 只写业务功能，不写约束；默认采用高成本方案而无替代方案 |
...
第一步：8 维度并行挖掘
    ...
    └─ 阶段 H：隐性假设与验证
    ↓
第一步 ter：实现视角七要素（数据源/数据流/定义/复用/成本反驳/开发疑问/DR 交接）
    ↓
...
**关键原则：** 每个步骤必须产出中间结果，用户确认后方可进入下一步。未确认禁止继续。RAModel 12 维是 8 维度挖掘的硬前置，不允许边写 RA 边补模型；实现视角七要素是 DR/Coding 的硬前置，由 G-RA-6 机器扫描。
...
必须逐类排查并写入 RA §9-quater.1：

| 来源类型 | 必查问题 | 证据 |
|----------|----------|------|
| DB 表 / view | 涉及哪些表、字段、读写方向、权威源？ | `ae-sdd assets query "<tableName>"` / DB schema |
| API / RPC | 调哪些接口，谁提供，是否已有 controller/client？ | assets api / 代码路径 |
| MQ / 事件 | 生产/消费哪些 topic/event，是否影响幂等？ | topic 定义 / 消费者代码 |
| 缓存 / Redis | 缓存 key、失效策略、一致性边界？ | cache 配置 / 代码路径 |
| 配置 / Nacos | 开关、阈值、租户配置、灰度配置？ | config key |
| 文件 / OSS | 上传/下载/解析/保留策略？ | 文件路径 / bucket |
| 第三方 | 三方 API、额度、失败重试、降级？ | 三方文档 / client |
| 前端/客户端状态 | localStorage、路由状态、表单状态、埋点？ | 前端代码路径 |
| 日志/审计/指标 | 可观测、审计、告警数据源？ | log/metric/tracing 定义 |

### I2 数据流链路

RA §9-quater.2 必须画出或列出：
...
RA §9-quater.3 必须消除开发者歧义：

| 类型 | 必须定义 |
|------|----------|
| 业务术语 | 名称、边界、与相近概念的区别 |
| 字段语义 | 类型、单位、空值含义、默认值、唯一性 |
| 状态/枚举 | 合法值、非法值、状态迁移、不变量 |
| ID 规则 | 生成方、幂等键、跨系统映射 |
| 权威源 | 冲突时以哪个系统/表/接口为准 |

### I4 现有实现 / 复用证据

RA §9-quater.4 必须写出实现证据，而不是只写"可复用"：

| 对象 | 必填证据 | 结论 |
|------|----------|------|
| 后端类/方法 | `src/.../Class.method` / `git grep` 结果 | 复用 / 改造 / 新建 / 不复用 |
| 表/API/MQ/cache | 表名、接口路径、topic、key pattern | 复用 / 改造 / 新建 / 不复用 |
| 前端组件 | 组件路径、页面路由、状态管理位置 | 复用 / 改造 / 新建 / 不复用 |
| 规则/策略 | 已有规则类、配置项、灰度开关 | 复用 / 改造 / 新建 / 不复用 |

### I5 高成本 / 难实现设计反驳

RA §9-quater.5 必须主动反驳高成本设计：

| 设计 | 为什么成本高/难实现 | 是否必须 | 不采用理由 | 低成本替代 |
|------|---------------------|----------|------------|------------|
| {方案} | {影响面/迁移/一致性/性能/团队成本} | 是/否 | {理由} | {替代方案} |

**强制态度：** 需求分析不是照单全收。若 PRD 暗含"重建平台/全量实时同步/跨域强一致/一次性大迁移/重写核心链路"等设计，RA 必须给出反驳或分阶段替代。

### I6 开发者疑问答复矩阵

RA §9-quater.6 必须站在写 DR/Coding 的人视角列问题：

| 开发者问题 | 答案 | 证据 | 状态 | 是否阻断 DR |
|------------|------|------|------|------------|
| {数据从哪里来？} | {答案} | {证据} | 已解决/待确认 | 是/否 |
| {失败如何补偿？} | {答案} | {证据} | 已解决/待确认 | 是/否 |

任何"待确认且阻断 DR=是"的问题必须进入第二步缺口管理，不能留在本表里假装已解决。

### I7 DR 生成交接包

RA §9-quater.7 必须给 DR 可直接引用的输入：

| DR 输入 | 必须交付 |
|---------|----------|
| 接口 | API/RPC/event/job 的路径、入参、出参、错误码 |
| 数据模型 | 表、字段、索引、缓存 key、MQ topic、配置 key |
| 状态/事务 | 状态机、事务边界、一致性、幂等、补偿 |
| 非功能 | 性能、安全、权限、审计、可观测、容量 |
| 测试验收 | 主流程、异常、边界、衍生 AC、数据一致性验证 |
| 发布迁移 | 灰度、迁移、回滚、开关、监控 |
...
> **范围：** §2 角色 / §3 场景 / §4 流程 / §5 数据 / §6 规则 / §7 设计方向 / §8 AC / §9 假设 / §9-quater 实现视角 — 所有结论。
...
1. 重跑 RA-G01~RA-G16 全部 16 道闸（不许只跑上轮失败的）
    ↓
2. 8 维度 + 实现视角回扫：阶段 A-H + 第一步 ter 各做一次"还能不能再挖出新的"
   （角色遗漏？场景组合？衍生规则 R'？跨域级联？隐性假设？数据源/数据流/复用证据/成本反驳/开发疑问遗漏？）
    ↓
3. 判定本轮有无新增确认缺口/缺陷
    ├─ 有新增（🔴/🟠/🟡 任一）→ 计数器归零
...
| 11.7 | 第一步-ter | 🆕 实现视角七要素 | §9-quater 数据源/数据流/定义/复用证据/成本反驳/开发者疑问/DR交接包（G-RA-6 I1-I7 校验）| §第一步 ter |
