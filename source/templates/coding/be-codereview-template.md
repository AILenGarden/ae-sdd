...
### 2.2 AC × 实现 × 测试 对照表（🔴 必填，每个 AC 一行） `必填`
...
## 3. 数据库逻辑链（🔴 阶段 C 评审产物） `选填（有数据库行为时必填）`

### 3.0 DB 行为全量清单（必查，禁止遗漏） `选填（有数据库行为时必填）`

| # | 操作类型 | 目标表 | 具体行为 | 实现类 → 方法 | Mapper 方法 | 事务边界 | 并发控制 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | | | | | | | |

### 3.1 涉及的表结构（必符合项目资产 §6.5） `选填（有数据库行为时必填）`
...
### 3.2 写操作链路（写明 SQL + WHERE + 乐观锁） `选填（有数据库行为时必填）`
...
## 4. 接口与外部依赖 `选填（有 SPI/外部调用时必填）`

### 4.1 对外 SPI 接口清单 `选填（有 SPI/外部调用时必填）`

| 接口 | 类路径 | 方法 | 入参 | 出参 | 幂等 | 调用方 |
| --- | --- | --- | --- | --- | --- | --- |
| | | | | | | |

### 4.2 外部服务调用 `选填（有 SPI/外部调用时必填）`

| # | 服务名 | 接口 | 调用时机 | 参数 | 异常策略 |
| --- | --- | --- | --- | --- | --- |
| | | | | | |

### 4.3 被调用接口（调用方是本 Story） `选填（有 SPI/外部调用时必填）`
...
## 6. 关键设计决策说明 `选填（有需说明的决策时必填）`

### 6.1 为什么这样分层？ `选填（有需说明的决策时必填）`

> {说明架构选型理由}

### 6.2 并发控制方案 `选填（有需说明的决策时必填）`

| 问题 | 方案 | 选择理由 | 实现位置 |
| --- | --- | --- | --- |
| | | | |

### 6.3 事务边界 `选填（有需说明的决策时必填）`

| 方法 | 事务范围 | 事务外操作 | 失败策略 |
| --- | --- | --- | --- |
| | | | |

### 6.4 与约束规范的对照（项目资产 §6） `选填（有需说明的决策时必填）`
...
## 7. CodeReview 自审结论 `必填`

### 7.1 问题清单（🔴 阻断型 / 🟠 严重 / 🟡 一般 / 🟢 建议） `必填`

| # | 检查项 | 等级 | 问题描述 | 修复建议 | 状态 |
| --- | --- | --- | --- | --- | --- |
| 1 | | 🔴/🟠/🟡/🟢 | | | Open/Fixed |

### 7.2 七道闸结果汇总（🔴 全部 ✅ 才能出报告） `必填`
...
### 7.3 阶段 D 评审：Test Review 结果引用核查 `必填`
...
## 8. 本次提交 Git 文件清单（按项目资产分层结构排序） `必填`
...
### 10.4 字段链路影响分析（如涉及 §6 全链路映射） `选填（有 🔴/🟠 问题时必填）`

- 字段 X 改动：触入 HTTP ✓ / BFF ✓ / SPI ✓ / DO ✓ / DB ✓（5 层都要同步）
- 字段 Y 改动：仅 DB 层（无需同步）

### 10.5 触发路径 `选填（有 🔴/🟠 问题时必填）`
...
### 10.6 执行后验收 `选填（有 🔴/🟠 问题时必填）`

- 7 道闸重新过一遍
- 6 阶段评审重新跑
- 连续 3 轮无新增问题才退出循环（3 轮仍有 🔴 → 升级用户）

### 10.7 用户确认 `选填（有 🔴/🟠 问题时必填）`
...
## 附录 A：填示例（🔴 仅作参考，填写时按本 Story 实际数据替换） `选填`
