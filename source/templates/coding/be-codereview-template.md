...
| 1.1 | └ Story 信息 | 🔴 必填 | 全部 |
| 1.2 | └ 业务理解摘要 | 🔴 必填 | 全部 |
| 2 | 核心业务逻辑详解（阶段 A 评审产物） | 🔴 必填 | 全部 |
| 2.1 | └ 主流程（按调用链路描述） | 🔴 必填 | 全部 |
| 2.2 | └ AC × 实现 × 测试 对照表 | 🔴 必填 | 全部（每个 AC 一行） |
| 2.3 | └ 状态机流转规则（如有） | 🔴 必填 | 全部 |
| 2.4 | └ 核心业务规则清单 | 🔴 必填 | 全部 |
| 2.5 | └ 阶段 A 评审结论 | 🔴 必填 | 全部 |
| 3 | 数据库逻辑链（阶段 C 评审产物） | 🟡 选填（条件） | 有数据库行为时 |
| 3.0 | └ DB 行为全量清单（必查，禁止遗漏） | 🟡 选填（条件） | 有数据库行为时 |
| 3.1 | └ 涉及的表结构（必符合项目资产 §6.5） | 🟡 选填（条件） | 有数据库行为时 |
| 3.2 | └ 写操作链路（写明 SQL + WHERE + 乐观锁） | 🟡 选填（条件） | 有数据库行为时 |
| 3.3 | └ 阶段 C 评审结论 | 🟡 选填（条件） | 有数据库行为时 |
| 4 | 接口与外部依赖 | 🟡 选填（条件） | 有 SPI/外部调用时 |
| 4.1 | └ 对外 SPI 接口清单 | 🟡 选填（条件） | 有 SPI/外部调用时 |
| 4.2 | └ 外部服务调用 | 🟡 选填（条件） | 有 SPI/外部调用时 |
| 4.3 | └ 被调用接口（调用方是本 Story） | 🟡 选填（条件） | 有 SPI/外部调用时 |
| 5 | 分层实现清单（阶段 B 评审产物） | 🔴 必填 | 全部 |
| 5.X+ | └ 各项目层实现（按 §3 实际分层，每层一节） | 🔴 必填 | 全部 |
| 5.6 | └ 分层职责红线核查（4 大红线，违反 = 阻断型） | 🔴 必填 | 全部 |
| 5.7 | └ 阶段 E 评审结论（项目资产合规性） | 🔴 必填 | 全部 |
| 6 | 关键设计决策说明 | 🟡 选填（条件） | 有需说明的决策时 |
| 6.1 | └ 为什么这样分层？ | 🟡 选填（条件） | 有需说明的决策时 |
| 6.2 | └ 并发控制方案 | 🟡 选填（条件） | 有需说明的决策时 |
| 6.3 | └ 事务边界 | 🟡 选填（条件） | 有需说明的决策时 |
| 6.4 | └ 与约束规范的对照（项目资产 §6） | 🟡 选填（条件） | 有需说明的决策时 |
| 7 | CodeReview 自审结论 | 🔴 必填 | 全部 |
| 7.1 | └ 问题清单（🔴/🟠/🟡/🟢 分级） | 🔴 必填 | 全部 |
| 7.2 | └ 七道闸结果汇总（全部 ✅ 才能出报告） | 🔴 必填 | 全部 |
| 7.3 | └ 阶段 D 评审：Test Review 结果引用核查 | 🔴 必填 | 全部 |
| 7.4 | └ 阶段 F 评审：跨文档引用核查 | 🔴 必填 | 全部 |
| 7.5 | └ ⑦bis 对称性闸（追溯矩阵） | 🔴 必填 | 全部 |
| 7.6 | └ 整改方案（仅当有 🔴/🟠 问题时填写） | 🔴 必填 | 全部 |
| 7.7 | └ 自审结论汇总 | 🔴 必填 | 全部 |
| 8 | 本次提交 Git 文件清单（按项目资产分层结构排序） | 🔴 必填 | 全部 |
| 8.1 | └ 统计摘要 | 🔴 必填 | 全部 |
| 8.2+ | └ 各层级分节（按项目资产 §3 实际分层填写） | 🔴 必填 | 全部 |
| 8.3 | └ 变更类型汇总 | 🔴 必填 | 全部 |
| 8.4 | └ Git 提交命令 | 🔴 必填 | 全部 |
| 9 | 产出物对账（闸 6） | 🔴 必填 | 全部 |
| 10 | CodeReviewUpdatePlan（改代码前硬前置） | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.1 | └ 问题清单（按 🔴/🟠/🟡/🟢 分级） | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.2 | └ 存疑项（评审时不确定的事项） | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.3 | └ 更新计划（按修复路径分组） | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.4 | └ 字段链路影响分析（如涉及 §6 全链路映射） | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.5 | └ 触发路径 | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.6 | └ 执行后验收 | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| 10.7 | └ 用户确认 | 🟡 选填（条件） | 有 🔴/🟠 问题时 |
| — | 报告结束 | 🟡 选填（条件） | 全部（流程校验提醒） |
| 附录 A | 填示例（仅作参考，非正文） | 🟡 选填（条件） | 供填写时参考 |
...
| 产出物 | 实际路径（由 document-storage §1.3 推导，不硬编码） | 是否存在 | 与报告一致 |
| --- | --- | --- | --- |
...
## 10. CodeReviewUpdatePlan（🔴 改代码前硬前置）
