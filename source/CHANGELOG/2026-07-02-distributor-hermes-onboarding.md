...
| `scripts/distributors/__init__.py` | `DISTRIBUTORS` 注册表新增 import + 注册 `HermesDistributor`，顺序置于 zcode 之后、harness 之前（copytree 类都在 harness 前） |
...
- 纯新增分发目标，不改变现有 claude/codex/zcode/harness 4 个分发器的行为
...
- `python -c "from distributors import DISTRIBUTORS; print([d().name for d in DISTRIBUTORS])"` → `['claude', 'codex', 'zcode', 'hermes', 'harness']`
