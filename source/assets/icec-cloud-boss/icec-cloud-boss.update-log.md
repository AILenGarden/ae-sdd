...
|------|---|
| **状态** | `✅ done` |
| **变更类型** | 🆕 新增索引层（§A-§G 此前完全缺失，G-00 靠子串匹配侥幸通过） |
| **涉及章节** | §1 lastAuditedAt / §10 缺口新增 #11（✅ 已补）/ 新增 §A-§G 全部索引层 |
| **变更前** | 资产文件止于 §10 + 附录 A，无 §A-§G 索引层；G-00 门禁因 `§A`~`§G` 字符在正文表格里偶现而子串匹配通过，实质缺失 |
| **变更后** | 新增 §A 大纲（7 字段速览 + 19 章目录）/ §B 模块索引 9 行 / §C 字段索引（boss_user 主表 11 行 + 错误码 7 行）/ §D 组件索引 12 行 / §E API 索引 11 行 / §F 反向索引 25 词 / §G 读取 API（标注已由 `ae-sdd assets` 脚本实现，含调用示例）；§10 缺口 #11 标 ✅ 已补；lastAuditedAt 更新为 2026-06-24 |
| **原因/触发** | 用户指出"项目资产索引做得差"，要求对标 ES 思想（倒排索引/分词/相关性评分）并脚本化。本次补全 boss 资产索引层，配合 `tools/lib/assets_index.py` + `ae-sdd assets query` 脚本提供可执行查询能力 |
| **Reviewer** | 未审 |
| **关联 PR/Commit** | - |
