...
use ae_sdd_domain::{
    ...
    DeliverableContract, DeliverableRequirement, DesignRoute, EpochMillis, EventSequence,
    EventStoreId, EvidenceDigest, EvidenceId, EvidenceRef, FlowRunId, GateId, HostAckId,
    HostActionId, InputFingerprint, InstructionId, InventoryGeneration, JobId, LeaseId,
    OperationId, PolicyDigest, ProcessPhase, ProjectKey, ProjectPathScope, ProjectRelativePath,
    RequestId, ResultDigest, SampleSequence, SeriesRunId, SessionId, StateRevision, StoryId,
    ToolchainDigest, TurnId, TurnSequence, VerificationId, WorkItemId, WorkScale, WorkspaceId,
};
...
display_parse_adapter!(instruction_id, InstructionId);
display_parse_adapter!(flow_run_id, FlowRunId);
display_parse_adapter!(series_run_id, SeriesRunId);
...
counter_adapter!(epoch_millis, EpochMillis);
...
/// `Option<SeriesRunId>` for the two absent-on-first-attempt edges of
/// [`crate::SeriesRunProjection`].
///
/// Written out rather than generated by `display_parse_adapter!` because adding an
/// optional arm to that macro would emit an unused module for all eight of its
/// existing users.
pub(crate) mod optional_series_run_id {
    use super::*;

    pub(crate) fn serialize<S>(
        value: &Option<SeriesRunId>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    ...
    pub(crate) fn deserialize<'de, D>(deserializer: D) -> Result<Option<SeriesRunId>, D::Error>
    where
        D: Deserializer<'de>,
    {
        Option::<String>::deserialize(deserializer)?
            .map(|raw| SeriesRunId::from_str(&raw))
            .transpose()
            .map_err(de::Error::custom)
    }
}
...
pub(crate) mod task_kind {
    use super::*;
    use crate::intake::TaskKind;

    pub(crate) fn serialize<S>(value: &TaskKind, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        serializer.serialize_str(value.as_wire())
    }

    pub(crate) fn deserialize<'de, D>(deserializer: D) -> Result<TaskKind, D::Error>
    where
        D: Deserializer<'de>,
    {
        let raw = String::deserialize(deserializer)?;
        TaskKind::from_wire(&raw).ok_or_else(|| de::Error::custom("unknown task kind"))
    }
}

pub(crate) mod input_sources {
    use super::*;
    use crate::intake::InputSource;

    pub(crate) fn serialize<S>(value: &[InputSource], serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let values: Vec<&str> = value.iter().map(|source| source.as_wire()).collect();
        values.serialize(serializer)
    }

    pub(crate) fn deserialize<'de, D>(deserializer: D) -> Result<Vec<InputSource>, D::Error>
    where
        D: Deserializer<'de>,
    {
        Vec::<String>::deserialize(deserializer)?
            .iter()
            .map(|raw| {
                InputSource::from_wire(raw).ok_or_else(|| de::Error::custom("unknown input source"))
            })
            .collect()
    }
}
