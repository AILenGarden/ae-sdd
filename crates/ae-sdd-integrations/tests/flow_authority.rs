...
use ae_sdd_domain::{AgentRole, BootId, EventStoreId, LeaseId, ProjectRelativePath};
...
fn stored_pass_cannot_bypass_the_durable_transition_intent() {
    ...
    request.fencing_token = Some(3);
    request.lease_id = Some(Uuid::from_u128(15).to_string());
...
fn fresh_native_gate_and_lease_commit_only_the_nested_story_phase() {
    ...
    let persistence = Arc::new(
        SqliteRuntimePersistence::open(&database).expect("runtime persistence"),
    );
    ...
    let authority: Value = serde_json::from_slice(
        &fs::read(root.path().join(".auto-engineering/flow-test/state.json"))
            .expect("state bytes"),
    )
    .expect("state JSON");
    ...
    assert_eq!(authority["storyStates"]["STORY-FLOW-001"]["phase"], "test-running");
