...
| **母版根 `SKILL.md`**（仓库顶层入口） | 让 `git clone` / GitHub 浏览者一眼看到的 SKILL 入口 | **不手工改；由 `bash scripts/sync-to-plugin.sh` 从 `skills/orchestration/ae-sdd-skill.md` 复制刷新** |
| `SKILL.md`（任一目标下） | 插件/本地运行入口 | 不手工改；由 `skills/orchestration/ae-sdd-skill.md` 复制生成 |
...
4. 确认同步目标目录（**三个**，由同一次脚本调用产出）：
   ...
   ```text
   1) <仓库根>/SKILL.md                                    # 母版根入口（git clone/GitHub 首屏可见）
   2) %USERPROFILE%/.claude/skills/ae-sdd/skills/ae-sdd   # 本地 Claude skills 安装目录
   3) <仓库根>/plugins/ae-sdd                              # marketplace plugin 副本
   ```

5. 确认所有三个目标下的 `SKILL.md` 均由最新 `skills/orchestration/ae-sdd-skill.md` 复制生成（包括母版根）。
...
- 脚本职责：
  1. **刷新母版根 `SKILL.md`**：从 `skills/orchestration/ae-sdd-skill.md` 复制到仓库根 `SKILL.md`（让 `git clone` / GitHub 首屏可见正确的 SKILL 入口）。
  2. 将 `skills/ae-sdd/` 母版整树覆盖同步到本地 Claude skills 安装目录 `%USERPROFILE%/.claude/skills/ae-sdd/skills/ae-sdd`。
  3. 同步生成仓库内的 marketplace plugin 副本 `plugins/ae-sdd/`（被 `.claude-plugin/marketplace.json` 引用）。
  4. 两个 DST 目标的入口 `SKILL.md` 均由最新的 `skills/orchestration/ae-sdd-skill.md` 覆盖生成（与母版根 SKILL.md 同源）。
...
| 禁止 | 原因 | 正确做法 |
|------|------|---------|
| 只改 `plugins/ae-sdd/` 或本地 Claude skills 目录 | 产物会被下次同步覆盖，且母版丢失变更 | 只改 `skills/ae-sdd/` 母版 |
...
| 手工编辑 `SKILL.md` | `SKILL.md` 是入口产物，会由 `ae-sdd-skill.md` 覆盖生成 | 修改 `ae-sdd-skill.md` 后同步 |
