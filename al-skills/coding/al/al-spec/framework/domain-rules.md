...
Project resources define repository facts, constraint references, templates,
storage and naming conventions, stricter checks, and approved deviations for
one exact `project_id`. A project template does not imply a project SKILL. It is
loaded only after the repository identity is confirmed; a project SKILL exists
only when the registry explicitly registers one for that type.
