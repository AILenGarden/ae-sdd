...
When `.al-knowledge/` exists, use ae-sdd's built-in knowledge capability to locate relevant context. Read `capabilities/al-knowledge/references/consumption-contract.md` relative to the ae-sdd entry; the user need not invoke or install a separate knowledge Skill. If ae-sdd or project knowledge is absent, inspect source directly. Knowledge does not replace implementation inspection or the authoritative Spec. Querying is read-only; update knowledge only within the current maintenance authorization.
