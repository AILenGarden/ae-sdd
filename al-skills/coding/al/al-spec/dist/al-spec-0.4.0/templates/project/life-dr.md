# LIFE-DR-{number}-{title}

## 使用说明

本模板用于 `icec-cloud-life` 项目的独立技术方案文档（DR）。调用
`life-dr` SKILL 时必须加载本模板；它提供文档结构，写作规范以 SKILL.md
为准。相关规格可以作为可选参考，不构成上下游前置依赖。

**DR 的职责：**
- 记录目标、范围、约束、架构边界、数据、接口、故障、安全、运维和发布设计
- 可选地列出行为拆分和验证策略，便于读者理解设计边界

**编写原则：**
- 功能流程细节不属于 DR 范畴，可在独立行为文档中描述
...
- 文档类型：DR 技术方案
- DR ID：LIFE-DR-{number}
- 项目前缀：{project-prefix}（用于项目内命名，如：`2c-im`）
- 可选相关规格：[{标题}]({文件路径})
- 作者：
- 创建日期：
- 最后更新：
...
> 示例：DR ID: LIFE-DR-001，项目前缀: 2c-im，相关规格（可选）：[背景说明](../../context.md)，创建日期: 2026-05-14，状态: Draft
...
## 11. 权限、安全与审计 `按需：涉及新权限模型或敏感数据时填写`
...
**拆分准则：**
- **独立边界**：每个 Story 都有明确范围、非范围、责任和验收标准
- **可独立交付**：每个 Story 都能单独描述和验证，不要求另一份 Story 先完成
- **协作透明**：需要协作时记录接口或协作关系，但不把另一份 Story 作为前置条件
- **前后端分离**：同一功能的前端和后端可拆成两个独立行为单元，分别描述各自责任
- **粒度参考**：单个行为单元预计开发时间 **≤4 天**（超过时记录拆分理由）

**行为 ID 命名规范：**
- 后端 Story：`STORY-{story-number}-BE`，如 `STORY-001-BE`
- 前端 Story：`STORY-{story-number}-FE`，如 `STORY-001-FE`
- 纯后台任务（无前端）：`STORY-{story-number}-BE`

**粒度要求：** 可选列出本 DR 涉及的行为单元，说明每个单元的范围边界、责任模块和预计工作量。该清单用于说明设计覆盖范围，不要求创建或先完成任何其他规格。

### 后端 Story

| Story ID | 标题 | 优先级 | 核心范围 | 责任模块 | 预计开发时间 |
| --- | --- | --- | --- | --- | --- |
| STORY-001-BE | | P1 | | | |
| STORY-002-BE | | P2 | | | |
...
flowchart TD
  ...
  STORY-001-BE -. optional collaboration .- STORY-002-BE
```

> 仅在有助于说明边界或协作顺序时填写；虚线表示可选协作关系，不表示规格前置依赖。

## 13. 测试策略 `按需：有特殊测试要求时填写，遵循 life-dr SKILL 中的测试规则`
...
## 15. 发布、迁移与回滚 `按需：有数据迁移或不兼容变更时填写`

- 发布步骤：（如需顺序，说明兼容性原因；不要把其他规格作为前置条件）
- 数据迁移：（是否需要，迁移脚本和验证方式）
- 回滚策略：（如何回滚，回滚后数据是否一致）
