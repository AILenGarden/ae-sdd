...
2. Load the current project constraints by reading the files under `constraints/` directly
   (indexed by [`constraints/README.md`](constraints/README.md)); a remembered or copied
...
5. Use strict RED-GREEN-REFACTOR, then run formatting, strict Clippy, focused/incremental tests
   only (the crate/module the change lands in plus matching test files; workspace-wide full
   regression runs only at release/distribution gates), real ae-sdd evidence, and independent
