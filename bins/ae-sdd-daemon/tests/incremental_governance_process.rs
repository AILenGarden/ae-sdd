...
async fn daemon_process_enforces_milestone_chain_and_denies_premature_completion() {
    ...
    for (i, path) in SLICES.iter().enumerate() {
        write_source(&fixture.project_root, path,
    ...
    let denied: Value = cli.call(RpcMethod::FlowNext, intent).await
    ...
    bind_write(&mut complete, &lease, read_revision(&fixture), "gov-premature-complete");
    let error = cli.call::<Value>(RpcMethod::OperationExecute, complete).await
        .expect_err("premature completion must be refused");
    let message = error.to_string();
    assert!(
        message.contains("ConfirmationRequired") || message.contains("GateBlocked")
...
async fn daemon_process_denies_stale_completion_after_changed_path_edit() {
    ...
    write_source(&fixture.project_root, SLICES[1], "pub fn beta() -> u8 { 42 }\n");
...
impl Fixture {
    fn new() -> Self {
        ...
        Self { root, project_root, runtime_dir, state_path }
    }
    fn allowed_root(&self) -> &Path { self.root.path() }
}
...
fn prepare_workspace(root: &Path) -> PathBuf {
    ...
    let verification: Vec<Value> = (1..=3).map(|i| json!({
...
struct DaemonProcess { child: Child }
...
impl DaemonProcess {
    async fn start(state_dir: &Path, allowed_root: &Path, policy_digest: &str) -> Self {
        ...
        cmd.arg("serve")
            ...
            .arg("--allowed-root").arg(allowed_root)
            .arg("--policy-digest").arg(policy_digest)
            .stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null());
        let mut child = cmd.spawn().expect("ae-sddd starts");
        let manifest = state_dir.join("endpoint.v1.json");
        let deadline = Instant::now() + Duration::from_secs(15);
        loop {
            if let Some(s) = child.try_wait().expect("status") { panic!("ae-sddd exited ({s})"); }
            if manifest.is_file() {
                let c = daemon_client(state_dir, ClientKind::Cli);
                if c.call::<Value>(RpcMethod::RuntimeStatus, params(json!({}))).await.is_ok() {
                    return Self { child };
                }
            }
    ...
    fn crash(&mut self) {
        if self.child.try_wait().expect("status").is_none() {
            self.child.kill().expect("kill"); self.child.wait().expect("wait");
        }
    }
}
...
#[derive(Clone)]
struct Identity {
    workspace_id: String, work_item_id: String, agent_id: String,
    session_id: String, capability_token: String,
}
...
fn milestone(fixture: &Fixture) -> String {
    read_state(fixture).pointer("/executionRuntime/completionMilestone")
        .and_then(Value::as_str).expect("milestone").to_owned()
}
...
async fn flow_next_action(client: &DaemonClient, identity: &Identity) -> Value {
    let projection: Value = client
        .call(RpcMethod::FlowSnapshot, identity.params(json!({})))
        .await
...
struct Lease { id: String, fencing: u64 }
...
async fn acquire_lease(client: &DaemonClient, identity: &Identity, key: &str) -> Lease {
    ...
    let result: Value = client.call(RpcMethod::OperationExecute, r).await
        .unwrap_or_else(|e| panic!("{key} lease: {e:?}"));
    Lease {
        id: result["data"]["leaseId"].as_str().expect("lease id").to_owned(),
        fencing: result["data"]["fencingToken"].as_u64().expect("fencing"),
    }
}

async fn release_lease(client: &DaemonClient, identity: &Identity, fixture: &Fixture, lease: &Lease, key: &str) {
    let mut r = identity.params(json!({
        "operation": "lease.release", "payload": {"owner":{"role":"root"}}
    }));
    bind_write(&mut r, lease, read_revision(fixture), key);
    let _: Value = client.call(RpcMethod::OperationExecute, r).await
        .unwrap_or_else(|e| panic!("{key} release: {e:?}"));
}

async fn record_evidence(client: &DaemonClient, identity: &Identity, fixture: &Fixture, lease: &Lease, prefix: &str) -> String {
    let mut r = identity.params(json!({
    ...
    bind_write(&mut r, lease, read_revision(fixture), &format!("{prefix}-evidence-record"));
    let result: Value = client.call(RpcMethod::OperationExecute, r).await
        .unwrap_or_else(|e| panic!("{prefix} evidence record: {e:?}"));
    assert_eq!(result["changed"], true, "{result}");
    result["data"]["evidenceId"].as_str().expect("evidence id").to_owned()
}

async fn finalize_evidence(client: &DaemonClient, identity: &Identity, fixture: &Fixture, lease: &Lease, prefix: &str) {
    let mut r = identity.params(json!({"operation": "evidence.finalize", "payload": {}}));
    bind_write(&mut r, lease, read_revision(fixture), &format!("{prefix}-evidence-finalize"));
    let result: Value = client.call(RpcMethod::OperationExecute, r).await
...
async fn register_and_cut_over(cli: &DaemonClient, admin: &DaemonClient, fixture: &Fixture) -> WorkspaceResult {
    ...
    let registered = cli.call::<WorkspaceResult>(RpcMethod::WorkspaceRegister, register).await.expect("registers");
    ...
    admin.call::<Value>(RpcMethod::RuntimeDrain, drain).await.expect("drains");

    let parity = WorkspaceParityEvidence {
        comparison_count: 1, mismatch_count: 0, source_revision: 1,
        legacy_digest: "a".repeat(64), rust_digest: "a".repeat(64),
    ...
    let cut = admin.call::<WorkspaceResult>(RpcMethod::WorkspaceModeTransition, transition).await.expect("cutover");
...
async fn open_root(client: &DaemonClient, workspace: &WorkspaceResult, key: &str) -> SessionResult {
    let mut r = params(json!({"externalKey": EXTERNAL_SESSION_KEY, "role": "root", "engaged": true}));
    ...
    r.idempotency_key = Some(key.to_owned());
    client.call(RpcMethod::SessionOpen, r).await.expect("root session")
}

fn daemon_client(state_dir: &Path, kind: ClientKind) -> DaemonClient {
    DaemonClient::new(state_dir.join("endpoint.v1.json"), kind,
        Arc::new(LocalIpcTransport), Duration::from_secs(5))
}

fn params(payload: Value) -> RequestParams<Value> {
    RequestParams {
        protocol_version: PROTOCOL_VERSION_V1.to_owned(),
        workspace_id: None, agent_id: None, session_id: None,
        capability_token: None, turn_id: None, work_item_id: None,
        lease_id: None, fencing_token: None, expected_revision: None,
        idempotency_key: None, confirmation: None, deadline_ms: 10_000, payload,
    }
}
