...
>
> **版本：** v3.8.2（🆕 2026-07-03：修复五层记忆存取断裂；新增 compact memory 写入硬约束；强化独立需求状态机：`state new --id --name` 创建 `{ID}--{name}` 目录，`state write` 默认跟随 active work item。历史变更见 `source/CHANGELOG/`。）
>
