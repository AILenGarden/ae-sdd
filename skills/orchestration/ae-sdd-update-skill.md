---
name: auto-engineering-update
description: 规范各 SKILL 的内容边界与维护规则。auto-engineering-skill 退守"流程编排"（流程怎么走、节点间如何流转），各子 SKILL 负责"环节内具体规则"（每一步具体怎么做、出错怎么处理）。当用户新增/修改任何 AE 相关 SKILL 时，先查阅本 SKILL 确认内容应放在哪个文件，避免在错误位置撰写或重复堆积。
---
...
| `plugins/ae-sdd/` | 发布/插件副本 | 不手工改；由发布流程从母版生成 |
| `%USERPROFILE%/.claude/skills/ae-sdd/skills/auto-engineering` | 本地 Claude skills 安装目录 | 不手工改；需要即时生效时由同步脚本覆盖 |
| `SKILL.md` | 插件/本地运行入口 | 不手工改；由 `ae-sdd-skill.md` 复制生成 |
...
4. 确认同步目标目录：
   ...
   ```text
   %USERPROFILE%/.claude/skills/ae-sdd/skills/auto-engineering
   ```

5. 确认本地运行入口已刷新：`SKILL.md` 应由最新 `ae-sdd-skill.md` 复制生成。
6. 在最终回复中明确说明：本次是否已执行同步脚本；如未执行，说明"仅修改母版，尚未同步到本地 Claude skills"。

### 同步脚本说明

- 脚本位置：`scripts/sync-to-plugin.sh`
- 脚本职责：将 `skills/ae-sdd/` 母版覆盖同步到本地 Claude skills 安装目录，并将 `ae-sdd-skill.md` 复制为 `SKILL.md`。
- 脚本不负责维护仓库内 `plugins/ae-sdd/` 副本；该目录仅在发布/打包流程中由母版生成。
